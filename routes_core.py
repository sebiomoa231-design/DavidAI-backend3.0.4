...
from david.conversations import count as count_conversations
...
async def status(current_user: Optional[dict] = Depends(get_current_user_optional)):
    ...
    return {
        ...
        "conversation_count": count_conversations(scoped_user_id),
...
async def overview(current_user: Optional[dict] = Depends(get_current_user_optional)):
    return await status(current_user)


@router.get("/api/dashboard/summary")
async def dashboard_summary(current_user: Optional[dict] = Depends(get_current_user_optional)):
    return await status(current_user)


@router.get("/api/system-overview")
async def system_overview(current_user: Optional[dict] = Depends(get_current_user_optional)):
    return await status(current_user)
